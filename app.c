...
static int get_pid_turn(int brightness);
static int now = 0;
static int last = 0;
    ...
    */
    while(1)
    {
        ...
        else
        {
            ...
            int red = rgb.r;
            if(rgb.b > 128 && rgb.g < 150) ev3_led_set_color(LED_ORANGE);
            else ev3_led_set_color(LED_GREEN);
            int brightness = red * 0.392;
            turn = get_pid_turn(brightness) * _EDGE;
        }
...
int get_pid_turn(int brightness) {
  last = now;
  now = brightness - (LIGHT_WHITE + LIGHT_BLACK) / 2;
